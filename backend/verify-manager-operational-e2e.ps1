# E2E Test: Manager Operational Endpoints After Invitation Acceptance
# This script tests that manager endpoints work correctly after accepting an invitation
...
Write-Host "=== Manager Operational Endpoints E2E Test ===" -ForegroundColor Cyan

# Step 0: Health Check
Write-Host "Step 0: Health Check" -ForegroundColor Cyan
try {
    $healthResult = Invoke-RobustCurl -Method "GET" -Url "http://localhost:3001/health"
    if ($healthResult.StatusCode -eq 200) {
        Write-Host "Health OK: OK" -ForegroundColor Green
    }
    else {
        throw "Health check failed with status $($healthResult.StatusCode)"
    }
}
catch {
...
# Step 5: Manager accepts the invitation
Write-Host "Step 5: Manager accepts invitation" -ForegroundColor Cyan
...
# Step 6A: Test Manager Dashboard
Write-Host "Step 6A: Manager Dashboard" -ForegroundColor Cyan
$dashboardResult = Invoke-ApiCall -Method "GET" -Url "$BaseUrl/manager/dashboard" -Token $managerToken -StepName "Manager Dashboard"

if (-not $dashboardResult.data) {
    throw "Manager dashboard missing data field"
}
Write-Host "Manager dashboard returned data successfully" -ForegroundColor Green

# Step 6B: Test Manager Tenants
Write-Host "Step 6B: Manager Tenants" -ForegroundColor Cyan
$tenantsResult = Invoke-ApiCall -Method "GET" -Url "$BaseUrl/manager/tenants" -Token $managerToken -StepName "Manager Tenants"

if ($tenantsResult.data -isnot [array]) {
    throw "Manager tenants should return an array"
}
Write-Host "Manager tenants returned array with $($tenantsResult.data.Count) items" -ForegroundColor Green

# Step 6C: Test Manager Invitations
Write-Host "Step 6C: Manager Invitations" -ForegroundColor Cyan
$invitationsResult2 = Invoke-ApiCall -Method "GET" -Url "$BaseUrl/manager/invitations" -Token $managerToken -StepName "Manager Invitations"

if ($invitationsResult2.data -isnot [array]) {
    throw "Manager invitations should return an array"
}
Write-Host "Manager invitations returned array with $($invitationsResult2.data.Count) items" -ForegroundColor Green

# Step 6D: Test Manager Leases
Write-Host "Step 6D: Manager Leases" -ForegroundColor Cyan
$leasesResult = Invoke-ApiCall -Method "GET" -Url "$BaseUrl/manager/leases" -Token $managerToken -StepName "Manager Leases"

if ($leasesResult.data -isnot [array]) {
    throw "Manager leases should return an array"
}
Write-Host "Manager leases returned array with $($leasesResult.data.Count) items" -ForegroundColor Green

Write-Host "========================================" -ForegroundColor Green
Write-Host "ALL MANAGER OPERATIONAL TESTS PASSED" -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Green
